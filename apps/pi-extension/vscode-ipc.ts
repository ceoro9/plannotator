...
type VSCodeIpcRegistry = Record<string, number>;
...
async function registeredPort(worktree: string): Promise<number | undefined> {
...
export async function isVSCodeEndpoint(port: number): Promise<boolean> {
	try {
		const response = await fetch(`http://127.0.0.1:${port}${PROBE_PATH}`, {
			signal: AbortSignal.timeout(500),
		});
		return response.status === 200 && (await response.text()) === PROBE_RESPONSE;
	} catch {
		return false;
	}
}
...
async function waitForEndpoint(worktree: string): Promise<number> {
	const deadline = Date.now() + REGISTRATION_TIMEOUT_MS;
	do {
		const port = await registeredPort(worktree);
		if (port !== undefined && (await isVSCodeEndpoint(port))) return port;
...
async function resolveEndpoint(worktree: string): Promise<number> {
	const port = await registeredPort(worktree);
	if (port !== undefined && (await isVSCodeEndpoint(port))) return port;
	const launch = await openFileInApp(worktree, "vscode");
	if (!launch.ok) throw new Error(`VS Code could not be launched: ${launch.error}`);
